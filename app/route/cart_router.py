...
from ..schema.cart_schema import AddToCartSchema, CartModel
from ..helper.cart_helper import add_to_cart, get_cart_items, remove_cart_items, update_cart_total_amount
from typing import List
router = APIRouter()


@router.post('/add-to-cart', response_model=List[CartModel], response_description="Add Item to cart")
...
@ router.get('/cart-items', response_model=List[CartModel], response_description="Get All items from cart")
async def getCartItems(user=Depends(get_current_user)):
    return await get_cart_items(user)


@ router.delete('/remove-cart-items/{item_id}', response_model=List[CartModel], response_description="Remove Item from cart")
